...
备份文件已迁移至 `tool_setting/sudo-backup/`（原位于 `PythonBasic/.reasonix/sudo-backup/`），包含：
...
**Why:** Codex 常通过 `sudo` 跨越 bwrap 沙箱写入系统目录（`/etc/`），一旦误改 `/usr/bin/sudo` 或 `/etc/sudoers` 的权限，sudo 就彻底不能用了，而修复 sudo 恰好又需要 root 权限——形成死锁。WSL 的 `wsl -u root` 是从 Windows 侧直接提权的手段，是唯一的 recovery path。备份在 `tool_setting/sudo-backup/` 可供恢复参考。

**How to apply:** sudo 报错（setuid/mode/syntax）时，先用 `wsl -u root` 恢复。备份文件在 `tool_setting/sudo-backup/`。
